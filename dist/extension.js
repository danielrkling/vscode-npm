...
let outputWindow;
const node_modules = vscode.Uri.joinPath(
...
function parseNpmPackage(packageNameString) {
  const regex = /^(?:@([^/]+)\/)?([^@]+)(?:@(.+))?$/;
...
async function installPackage(packageName, versionRange) {
  ...
  const version = semverExports.maxSatisfying(
  ...
  await vscode.workspace.fs.createDirectory(node_modules);
  const files = await parseTarGzip(await tarballResponse.arrayBuffer());
  for (const file of files) {
    const uri = vscode.Uri.joinPath(node_modules, packageName, file.name.slice(8));
